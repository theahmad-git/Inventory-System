...
#include "ui_salesreportform_new.h"
...

{
    ...
    connect(ui->pushButton_createBackup, &QPushButton::clicked,
            this, &salesreportform::on_pushButton_createBackup_clicked);
...
void salesreportform::onHeaderClicked(int logicalIndex)
    ...
    void salesreportform::on_pushButton_createBackup_clicked()
    {
        QString selectedOption = ui->comboBox_createBackup->currentText();
        QString tableName;
        ...
        if (tableName == "sales" || tableName == "product") {
            // Use the stored fromDate and toDate instead of calendarWidget's selected date
            if (!fromDate.isValid() || !toDate.isValid()) {
                QMessageBox::warning(this, "Error", "Please select both From and To dates first");
                return;
            }

            QDateTime fromDateTime(fromDate, ui->timeEdit_from->time());
            QDateTime toDateTime(toDate, ui->timeEdit_to->time());
            ...
            exportToExcel(tableName, fromDateTime, toDateTime);
        } else {
            exportToExcel(tableName);
        }
    }

void salesreportform::exportToExcel(const QString &tableName)
    {
        QSqlQuery query(QString("SELECT * FROM %1").arg(tableName));
        exportQueryToExcel(query, tableName + "_backup");
    }
                                        ...
                                        const QDateTime &to)
    {
        if (!from.isValid() || !to.isValid()) {
            QMessageBox::warning(this, "Invalid Date",
                                 "Please select valid From and To date/time before exporting.");
            return;
        }
        ...
        QString queryStr = QString("SELECT * FROM %1 WHERE ").arg(tableName);

        if (tableName == "sales") {
            queryStr += QString("sale_date_time BETWEEN '%1' AND '%2'")
            .arg(from.toString("yyyy-MM-dd HH:mm:ss"))
                .arg(to.toString("yyyy-MM-dd HH:mm:ss"));
        } else if (tableName == "product") {
            queryStr += QString("last_updated BETWEEN '%1' AND '%2'")
            .arg(from.toString("yyyy-MM-dd HH:mm:ss"))
                .arg(to.toString("yyyy-MM-dd HH:mm:ss"));
        } else {
            // For other tables, just export all rows (no date filtering)
            queryStr = QString("SELECT * FROM %1").arg(tableName);
        }

        QSqlQuery query(queryStr);
        QString fileName = tableName + "_report_" + from.toString("yyyyMMdd") + "_to_" + to.toString("yyyyMMdd");
        exportQueryToExcel(query, fileName);
    }


void salesreportform::exportQueryToExcel(QSqlQuery &query, const QString &baseFileName)
    {
        if (!query.exec()) {
        ...
        QString filePath = QFileDialog::getSaveFileName(
            this,
            "Save Excel File",
            QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + "/" + baseFileName + ".xlsx",
            "Excel Files (*.xlsx)");
        ...
        // Write headers (bold formatting)
        for (int col = 0; col < record.count(); ++col) {
            QXlsx::Format headerFormat;
            headerFormat.setFontBold(true);
            xlsx.write(1, col+1, record.fieldName(col), headerFormat);
        }
        ...
        while (query.next()) {
            for (int col = 0; col < record.count(); ++col) {
                ...
                if (record.field(col).type() == QVariant::DateTime) {
                    QDateTime dt = query.value(col).toDateTime();
                    xlsx.write(row, col+1, dt);
                } else {
        ...
        // Auto-size columns
        for (int col = 0; col < record.count(); ++col) {
            xlsx.setColumnWidth(col+1, 15); // 15 characters wide
        }
...
QString salesreportform::browseExcelFile()
    {
        QString filePath = QFileDialog::getOpenFileName(
            this,  // Changed from nullptr to 'this' for proper parent
            "Select Excel File",
            QDir::homePath(),
        ...
        if (!filePath.isEmpty()) {
            readExcelFile(filePath);  // Call the read function after selection
        }
        return filePath;
    }
    // Fix the readExcelFile function
void salesreportform::readExcelFile(const QString& filePath)
    {

        QXlsx::Document xlsx(filePath);
        if (xlsx.load()) {
            QString value = xlsx.read(1, 1).toString();
            qDebug() << "Cell A1 value:" << value;  // Changed from UI label to debug output

            // Optional: Show in message box
            QMessageBox::information(this, "Excel Value",
                                     "Cell A1 contains: " + value);
        } else {
            qDebug() << "Failed to load Excel file!";
            QMessageBox::warning(this, "Error", "Failed to load Excel file!");
        }
    }

    QString salesreportform::getSaveFilePath(const QString &defaultName)
    {
        QString filePath = QFileDialog::getSaveFileName(
            this,
            tr("Save File"),
